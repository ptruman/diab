...
# Check healthcheck is not disabled - if DIAB_DISABLEHEALTHCHECK is not present, assume check should run
if [ -f $DIAB_DISABLEHEALTHCHECK ]; then
        ...
        # Healthcheck is disabled - exit healthy
        HealthState=0
fi
exit $HealthState
